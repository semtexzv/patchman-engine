...
func TestSyncErratas(t *testing.T) {
	utils.SkipWithoutDB(t)
	core.SetupTestEnvironment()

	var data vmaas.ErrataResponse

	assert.Nil(t, json.Unmarshal([]byte(testErratas), &data))
	...
	for i := 0; i < 2; i++ {
		assert.Nil(t, storeErratas(data.ErrataList))
		var count int

		assert.Nil(t, database.Db.Model(&models.ErrataMetaData{}).Where("url = ?", "TEST").Count(&count).Error)

		assert.Equal(t, count, len(data.ErrataList))
	}

	assert.Nil(t, database.Db.Unscoped().Where("url = ?", "TEST").Delete(&models.ErrataMetaData{}).Error)
}
