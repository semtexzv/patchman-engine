...
# Initialize the databse, utilizing auth method from env variables
# in dev enfironments, this means that psql connects over local socket
# and in prod using environemnt variables set by run-rds
...
if [[ $DB_INITIALIZED == "0" ]]; then
  ...
  psql -d ${DB_NAME} -f ${CONTAINER_SCRIPTS_PATH}/start/create_users.sql
fi
